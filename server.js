...
app.post('/api/analizar', (req, res) => {
  ...
  const actoresNombres = Array.isArray(actores)
  ...
  if (skill === 'comparativo') {
    if (!actoresNombres || actoresNombres.length < 2) {
      return res.status(400).json({ error: 'La skill "comparativo" requiere al menos 2 actores.' });
    }
    if (actoresNombres.length > 6) {
      return res.status(400).json({ error: 'La skill "comparativo" admite máximo 6 actores.' });
    }
  }
...
async function procesarAnalisis({ jobId, skill, actorName, actor2Name, actoresNombres, mes, anio, APIFY_TOKEN, OPENROUTER_KEY }) {
  try {
    const esComparativo = skill === 'comparativo' && actoresNombres && actoresNombres.length >= 2;
    console.log(`[+] Iniciando análisis (${jobId}) para: ${esComparativo ? actoresNombres.join(' vs ') : actorName + (actor2Name ? ' vs ' + actor2Name : '')} (${skill})`);

    JOBS.set(jobId, { status: 'processing', progreso: 'Extrayendo fuentes en Apify (X, Facebook, Instagram, TikTok, YouTube, prensa)...' });

    // 1. Scraping masivo (en paralelo, uno o varios actores según la skill)
    let datosActor1 = null, datosActor2 = null, datosActores = null;
    if (esComparativo) {
      datosActores = await Promise.all(actoresNombres.map(n => scrapeActor(n, APIFY_TOKEN)));
    } else {
    ...
    const prompt = buildPrompt({ skill, actorName, actor2Name, actoresNombres, mes, anio, datosActor1, datosActor2, datosActores, schema });
    ...
    const normalized = normalizeResponse(structured, skill, actoresNombres);

    const fuentesEncontradas = esComparativo
      ? datosActores.reduce((sum, d) => sum + (d?.count || 0), 0)
    ...
    JOBS.set(jobId, {
      ...
      result: {
        ...
        actores: esComparativo ? actoresNombres : null,
        periodo: `${mes} ${anio}`,
        fuentesEncontradas,
        data: normalized,
      },
...
// =========================================================
function normalizeResponse(data, skill, actoresNombres) {
  ...
  // COMPARATIVO (multi-actor, 2-6)
  if (skill === 'comparativo') {
    const nombres = (actoresNombres && actoresNombres.length ? actoresNombres : ['Actor 1', 'Actor 2']);
    const PALETTE = ['#00A8B5', '#C0392B', '#D35400', '#1E8449', '#1B4F8A', '#C49A00'];
    ensureArray(data, 'actores', nombres.map((n, i) => ({ nombre: n, color: PALETTE[i % PALETTE.length] })));
    // Asegura que cada actor declarado tenga nombre y color, y respeta el orden/nombres enviados
    data.actores = nombres.map((n, i) => {
      const existente = (data.actores || []).find(a => a && a.nombre === n);
      return { nombre: n, color: (existente && existente.color) || PALETTE[i % PALETTE.length] };
    });
    ensureObject(data, 'alertaPrincipal', { actor: nombres[0], nivel: 'Sin datos', label: 'Sin datos' });
    ensureObject(data, 'periodo', { corte: 'Sin datos', rango: 'Sin datos' });
    if (!data.resumenKpis) data.resumenKpis = 'Sin datos suficientes.';
    ensureArray(data, 'kpiCards', [{ label: 'Sin datos', val: '—', sub: 'Sin datos suficientes', color: 'n' }]);
    ensureArray(data, 'npsPorActor', nombres.map(() => 0));
    if (!data.npsNote) data.npsNote = '';
    ensureArray(data, 'ratioPorActor', nombres.map(() => 0));
    if (!data.ratioNote) data.ratioNote = '';
    ensureObject(data, 'traSerie', { labels: ['Sin datos'], series: {} });
    if (!data.traSerie.labels) data.traSerie.labels = ['Sin datos'];
    if (!data.traSerie.series) data.traSerie.series = {};
    if (!data.traNote) data.traNote = '';
    ensureObject(data, 'sentimientoGeneral', {});
    nombres.forEach(n => { if (!data.sentimientoGeneral[n]) data.sentimientoGeneral[n] = [0, 0, 0, 0]; });
    ensureObject(data, 'sentimientoCruces', {});
    ['edad', 'genero', 'partido'].forEach(k => {
      ensureObject(data.sentimientoCruces, k, { segments: ['Sin datos'], data: {}, note: '' });
      if (!data.sentimientoCruces[k].data) data.sentimientoCruces[k].data = {};
    });
    ensureArray(data, 'topOfMindTabla', [['Sin datos', '—', '—']]);
    if (!data.topOfMindLead) data.topOfMindLead = '';
    ensureObject(data, 'topOfMindCruces', {});
    ['edad', 'genero', 'partido'].forEach(k => {
      ensureObject(data.topOfMindCruces, k, { segments: ['Sin datos'], themes: ['Sin datos'], data: {}, note: '' });
    });
    if (!data.picosLead) data.picosLead = '';
    ensureArray(data, 'picosTabla', [['—', 'Sin datos', nombres[0], '—']]);
    ensureObject(data, 'picosSerie', { labels: ['Sin datos'], series: {} });
    ...
    ensureArray(data, 'nubePalabras', [{ word: 'sin datos', weight: 50, sentiment: 'neutro' }]);
    ...
    ensureArray(data, 'alertaTabla', nombres.map(n => [n, 'Sin datos', 'Sin datos suficientes.']));
    ...
    ensureArray(data, 'territorialTabla', [['—', 'Sin datos', '—']]);
    if (data.territorialAlerta === undefined) data.territorialAlerta = null;
  }
...
const SCHEMAS = {
  ...
  comparativo: JSON.stringify({
    actores: [{ nombre: "string (debe coincidir EXACTO con los nombres de actores dados)", color: "#hex" }],
    alertaPrincipal: { actor: "string", nivel: "NIVEL 1|NIVEL 2|NIVEL 3|NIVEL 4", label: "string (ej. ALERTA AMARILLA)" },
    periodo: { corte: "string (ej. 18 ago 2026)", rango: "string (ej. 9–18 ago 2026, 10 días)" },
    resumenKpis: "string",
    ...
    ratioPorActor: [0],
    ratioNote: "string",
    traSerie: { labels: ["string"], series: { "NOMBRE_ACTOR": [0] } },
    traNote: "string",
    sentimientoGeneral: { "NOMBRE_ACTOR": [0, 0, 0, 0] },
    sentimientoCruces: {
      edad: { segments: ["18-29", "30-44", "45-59", "60+"], data: { "NOMBRE_ACTOR": { "18-29": [0, 0, 0, 0] } }, note: "string" },
      genero: { segments: ["Mujer", "Hombre"], data: { "NOMBRE_ACTOR": { "Mujer": [0, 0, 0, 0] } }, note: "string" },
      partido: { segments: ["string"], data: { "NOMBRE_ACTOR": { "SEGMENTO": [0, 0, 0, 0] } }, note: "string" }
    },
    topOfMindTabla: [["tema", "peso relativo", "actor(es) que lo capitaliza(n)"]],
    topOfMindLead: "string",
    topOfMindCruces: {
      edad: { segments: ["18-29", "30-44", "45-59", "60+"], themes: ["string"], data: { "TEMA": [0, 0, 0, 0] }, note: "string" },
      genero: { segments: ["Mujer", "Hombre"], themes: ["string"], data: { "TEMA": [0, 0] }, note: "string" },
      partido: { segments: ["string"], themes: ["string"], data: { "TEMA": [0] }, note: "string" }
    },
    picosLead: "string",
    picosTabla: [["fecha", "evento", "NOMBRE_ACTOR", "efecto"]],
    picosSerie: { labels: ["string"], series: { "NOMBRE_ACTOR": [0] } },
    plataformasLead: "string",
    plataformasRadar: { labels: ["X (Twitter)", "Facebook", "Instagram", "Medios digitales"], data: { "NOMBRE_ACTOR": [0, 0, 0, 0] } },
    ...
    nubeNota: "string",
    hashtags: [["hashtag", "actor", "tono", "plataforma", "origen", 0]],
    hashtagsNota: "string",
    ...
    riesgos: [["ALTO|MEDIO|BAJO", "titulo", "texto", "bivariado"]],
    oportunidades: [["ALTA|MEDIA|BAJA", "titulo", "texto", "bivariado"]],
    alertaTabla: [["NOMBRE_ACTOR", "nivel de alerta emoji+texto", "justificación breve"]],
    escenarioSube: "string",
    escenarioBaja: "string",
    territorialLead: "string",
    territorialTabla: [["región/municipio", "NOMBRE_ACTOR con mayor presencia", "nota"]],
...
function buildPrompt({ skill, actorName, actor2Name, actoresNombres, mes, anio, datosActor1, datosActor2, datosActores, schema }) {
  const esComparativo = skill === 'comparativo' && actoresNombres && actoresNombres.length >= 2;

  let contexto;
  if (esComparativo) {
    contexto = actoresNombres
      .map((nombre, i) => `--- Datos crudos sobre ${nombre} ---\n${resumirFuentes(datosActores[i])}`)
      .join('\n\n');
    contexto = `Actores a comparar (en este orden EXACTO): ${actoresNombres.join(', ')}\n\n${contexto}`;
  } else {
  ...
  const instruccionesEstructura = skill === 'emociones'
    ...
    : esComparativo
    ? `\nINSTRUCCIONES DE ESTRUCTURA CRÍTICAS (Comparativo):\n- Debes generar datos para EXACTAMENTE estos ${actoresNombres.length} actores, en este orden: ${actoresNombres.join(', ')}. No agregues ni omitas actores.\n- En todos los objetos con forma { "NOMBRE_ACTOR": [...] }, usa como llave el nombre EXACTO del actor (idéntico al proporcionado), una entrada por cada uno de los ${actoresNombres.length} actores.\n- "npsPorActor" y "ratioPorActor" deben ser arrays numéricos con exactamente ${actoresNombres.length} elementos, en el mismo orden que la lista de actores.\n- Los arrays de sentimiento [positivo, neutro, negativo, polarizado] deben sumar aproximadamente 100.\n- "alertaTabla" debe tener exactamente una fila por cada uno de los ${actoresNombres.length} actores.\n- Asigna a cada actor en "actores" un color hexadecimal distinto y visualmente diferenciable.`
    : '';
