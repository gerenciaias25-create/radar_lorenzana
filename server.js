...
function normalizeResponse(data, skill, ctx = {}) {
  ...
  if (skill === 'tensiones') {
    ...
    // Semáforo de Inteligencia: 5 indicadores fijos del panel superior de la
    // pestaña Tensiones (Tensión Social, Reputacional, Escalamiento,
    // Confianza Institucional, Movilización Social).
    ensureArray(data, 'semaforo', []);
    ensureArray(data, 'emociones', []);
    // Evolución vs. periodo anterior: una entrada por cada emoción de
    // "emociones" (mismo orden/nombre) con su tendencia y detalle.
    ensureArray(data, 'emocionesTendencia', []);
    if (!data.emocionesLectura) data.emocionesLectura = 'Sin lectura política estratégica disponible.';
    ensureArray(data, 'narrativas', []);
    ensureArray(data, 'territorios', []);
    ensureArray(data, 'riesgos', []);
    ensureArray(data, 'trayectoria', []);
    // Etiquetas de los 5 cortes mensuales que encabezan la tabla/gráfica de
    // Trayectoria (ej. ["Mayo","Junio","Julio","Agosto","Sept (corte 14)"]).
    ensureArray(data, 'trayectoriaMeses', ['Periodo 1', 'Periodo 2', 'Periodo 3', 'Periodo 4', 'Actual']);
    ensureObject(data, 'aprobacionAlcalde', { valor: 0, periodo: 'Sin dato', fuente: '', nota: 'Sin dato de aprobación verificado en encuesta pública.' });
...
const SCHEMAS = {
  ...
  tensiones: JSON.stringify({
    ...
    ranking: [
      {
        ...
        score: 0,
        color: "#C05621",
        nivel: "Crítico|Alto|Relevante|Medio|Bajo|Emergente",
        ...
        lecturaEstrategica: "string (párrafo de 80-140 palabras, EL CAMPO MÁS IMPORTANTE de cada tensión: conecta este hallazgo con otros hechos del periodo, explica qué patrón revela, qué actor queda expuesto y por qué esto importa políticamente ahora — no repitas literalmente lo dicho en 'evidencia', interprétalo)",
        recomendacion: "string (acción concreta y específica, no genérica)"
      }
    ],
    // Semáforo de Inteligencia: EXACTAMENTE 5 indicadores, en este orden fijo:
    // Tensión Social, Reputacional, Escalamiento, Confianza Institucional, Movilización Social.
    semaforo: [
      { indicador: "Tensión Social|Reputacional|Escalamiento|Confianza Institucional|Movilización Social", valor: "string (número 0-100 para los primeros tres, ej. '87'; etiqueta cualitativa para los últimos dos, ej. 'MEDIA-BAJA' o 'MEDIA')", banda: "string (ej. 'Roja / Crítica', 'Naranja / Alto', 'Verde / Favorable')", detalle: "string (una línea de contexto que sustente el valor, ej. 'Aprobación 50.5% (ago 2026)' o 'Sin protesta de calle documentada')" }
    ],
    emociones: [
    ...
    // Evolución vs. periodo anterior: UNA entrada por CADA emoción del array
    // "emociones" de arriba, mismo "nombre" y mismo orden.
    emocionesTendencia: [
      { nombre: "string (debe coincidir EXACTAMENTE con un 'nombre' de 'emociones')", tendencia: "string (EXACTAMENTE uno de: '↑ Fuerte ascenso', '↑ En ascenso', '↑ Sostenida alta', '— Estable alto', '— Estable frágil', '↓ En descenso', '↓ Ceden', '1 Creciente' — usa el símbolo de dirección apropiado)", detalle: "string (una línea que explique QUÉ concentra o sostiene esa tendencia, con hecho o territorio concreto)" }
    ],
    // Lectura política estratégica del mapa emocional completo (no de una sola emoción).
    emocionesLectura: "string (párrafo de 150-220 palabras que interprete, emoción por emoción -en negritas conceptuales-, cómo se combinan las emociones dominantes y qué implica para la estrategia política del actor evaluado)",
    narrativas: [
      { nombre: "string", tema: "string", actor: "string", politica: "string (impacto/potencial político de esta narrativa)", potencialScore: "0-100 (índice numérico de potencial político/de propagación de esta narrativa específica, usado en la gráfica de barras de la pestaña)", frase: "string (cita textual representativa)", fuente: "string (medio y fecha aproximada, ej. 'La Silla Rota, mayo-julio 2026')" }
    ],
    territorios: [
      { nombre: "string", nivel: "Crítico|Alto|Medio (nivel de sensibilidad; DEBE corresponder con 'color': Crítico=Rojo, Alto=Naranja, Medio=Amarillo)", tension: "string", emocion: "string", color: "Rojo|Naranja|Amarillo", observaciones: "string (párrafo de 40-70 palabras con evidencia documentada: qué ocurrió, cuándo, cifras concretas -personas/colonias/pesos- y fuente entre paréntesis; nunca solo descriptivo sin datos)" }
    ],
    riesgos: [
      { nombre: "string", srr: 0, accion: "string", consecuencia: "string (párrafo de 30-60 palabras: qué pasa concretamente si esta señal NO se atiende, con plazo aproximado, ej. 'Normalización de bloqueos y erosión del discurso de corredor logístico seguro en 30 días')", color: "#hex", actorExpuesto: "string", tipoSenal: "Amplificada legítima|Orgánica|Inducida|Aislada", probEscalar: "Alta|Media|Baja" }
    ],
    // Trayectoria: 5 cortes mensuales por tensión, alineados con "trayectoriaMeses".
    trayectoriaMeses: ["string (mes 1, ej. 'Mayo')", "string (mes 2)", "string (mes 3)", "string (mes 4)", "string (mes actual/corte, ej. 'Sept (corte 14)')"],
    trayectoria: [
      { nombre: "string", m1: 0, m2: 0, m3: 0, m4: 0, m5: 0, delta: "string (ej. '+7' o '-3', con signo; m5 menos m1)", tipo: "Persistente|Nueva|Emergente", velocidad: "Acelerada|Moderada|Gradual|Estable|Bajando" }
    ],
    // Aprobación del alcalde/actor: único dato verificado en encuesta pública (no interpolado).
    aprobacionAlcalde: { valor: "0-100 (número; % de aprobación en el único corte con encuesta pública verificada)", periodo: "string (ej. 'Agosto 2026')", fuente: "string (ej. 'Demoscopia Digital, agosto 2026')", nota: "string (una línea aclarando que solo hay un corte verificado y por qué no se interpolan los demás meses)" },
    alertas: [
      { titulo: "string", rows: [["clave", "valor"]], comentariosMedios: [{ cita: "string (comentario textual breve tomado o inferido de prensa/redes sobre este hecho, entre comillas)", fuente: "string (medio o red social + fecha aproximada, ej. 'Facebook, comentarios en nota de La Silla Rota, 15 ago 2026')" }] }
    ],
    socio: {
      radiografia: [
...
function buildPrompt({ skill, actorName, actor2Name, actoresNombres, datosPorActor, mes, anio, datosActor1, datosActor2, schema }) {
  ...
  const instruccionesEstructura = skill === 'emociones'
    ...
    : skill === 'tensiones'
    ? `\nINSTRUCCIONES DE ESTRUCTURA CRÍTICAS (Tensiones):\n- CONSISTENCIA ENTRE PESTAÑAS (crítico): "trayectoria" debe tener EXACTAMENTE las mismas tensiones que "ranking" (mismos "nombre", mismo orden), y "riesgos" también debe cubrir esas mismas tensiones en el mismo orden — un analista que lea las pestañas debe reconocer que hablan de las mismas 6-10 tensiones, no de conjuntos distintos. El campo "m5" (último corte) de cada fila en "trayectoria" debe ser IGUAL al "score" de esa misma tensión en "ranking".\n- "ranking[].emocion" formato EXACTO: "EmociónPrimaria + EmociónSecundaria · X/5" (ej. "Hartazgo + Desprotección · 4/5"), nunca solo una palabra suelta.\n- "ranking[].evidencia" es un párrafo (no una frase) que encadena 2-3 datos verificables (cifras, fechas, colonias) cada uno rematado con su fuente entre paréntesis, siguiendo este patrón: "Dato 1 con cifra y fecha (Fuente, ICF X.X) - Dato 2 (Fuente, ICF X.X)". Nunca lo dejes como una oración vaga sin cifras ni fuente.\n- "ranking[].lecturaEstrategica" es DISTINTO de "evidencia" y de "recomendacion": es la interpretación de inteligencia (por qué el hallazgo conecta con otros hechos del periodo/otras tensiones, qué patrón revela, qué actor queda expuesto) — nunca lo dejes como una repetición parafraseada de "evidencia".\n- "semaforo" es un array de EXACTAMENTE 5 elementos en este orden fijo: Tensión Social, Reputacional, Escalamiento, Confianza Institucional, Movilización Social. "Tensión Social" DEBE ser igual (o casi igual) al "score" de la tensión #1 del "ranking"; "Reputacional" debe ser coherente con el promedio de "srr" de "riesgos"; "Escalamiento" refleja qué tan cerca está la tensión #1 de agravarse; "Confianza Institucional" y "Movilización Social" usan etiquetas cualitativas (ALTA/MEDIA/MEDIA-BAJA/BAJA) con su "detalle" anclado en un dato real (encuesta, protestas documentadas o su ausencia).\n- "emociones[].descripcion" siempre debe indicar si la emoción es estructural/coyuntural y su tendencia (sostenida/en descenso/nueva), no solo repetir el nombre de la emoción.\n- "emocionesTendencia" debe tener UNA entrada por CADA emoción de "emociones" (mismo "nombre", mismo orden) — nunca una lista más corta ni con nombres que no existan en "emociones".\n- "territorios[].nivel" y "territorios[].color" deben ser coherentes entre sí: "Crítico"↔"Rojo", "Alto"↔"Naranja", "Medio"↔"Amarillo" — nunca combines un nivel con un color que no le corresponde.\n- "riesgos[].tipoSenal" debe ser EXACTAMENTE uno de: "Amplificada legítima", "Orgánica", "Inducida", "Aislada". "riesgos[].probEscalar" debe ser EXACTAMENTE "Alta", "Media" o "Baja". "riesgos[].consecuencia" NUNCA queda vacío: siempre con plazo aproximado (ej. "en 30-45 días").\n- "trayectoriaMeses" son EXACTAMENTE 5 nombres de mes/periodo reales y consecutivos (el 5º es el corte más reciente/actual) — "trayectoria[].m1..m5" deben alinearse en el mismo orden temporal. "delta" es un STRING con signo, ej. "+7" o "-3" (m5 menos m1), nunca un número sin signo ni una palabra. "velocidad" refleja qué tan rápido cambió entre m1 y m5 (Acelerada = salto grande en el último tramo, Gradual = cambio sostenido y lento, Estable = casi sin cambio).\n- "aprobacionAlcalde" reporta el ÚNICO corte de aprobación con encuesta pública real verificable en las fuentes crudas; si no existe ninguna, usa "valor":0 y explica en "nota" que no hay encuesta pública verificada para el periodo — NUNCA inventes ni interpoles una cifra de aprobación sin fuente.\n- "alertas[].rows": cada alerta necesita mínimo 8 filas cubriendo Territorio, Emoción, Actor expuesto, Qué ocurrió (párrafo con fecha), Narrativa activa, Fuente verificadora, Riesgo, Escalamiento, Acción inmediata — usa esas etiquetas o muy similares, en ese orden. "alertas[].comentariosMedios" añade 2-4 comentarios/reacciones de prensa o redes sociales sobre ese hecho específico, cada uno con su fuente y fecha aproximada — si las fuentes crudas no traen comentarios textuales, redacta reacciones plausibles y represéntalas como inferencia razonable, nunca como cita textual verificada inexistente.\n- \"narrativas[].potencialScore\" (0-100) debe ser coherente con el texto de \"politica\" de esa misma narrativa (un potencial descrito como alto/viral debe tener un score alto) y debe haber variación real entre narrativas, no todas en el mismo rango.\n- \"socio\" (pestañas Socioafectiva): las zonas de \"socio.radiografia[].tensionDominante\" deben coincidir con nombres de tensiones que existan en \"ranking\"; las emociones de \"socio.radiografia[].emocion\" y \"socio.segmentacion[].emocion\" deben ser coherentes con \"emociones\". \"socio.radiografia[].ias\" es 0-100. \"socio.segmentacion[].color\" EXACTAMENTE \"Rojo\", \"Naranja\", \"Amarillo\" o \"Verde\". No dupliques textualmente contenido de otras pestañas: aquí la lente es por ZONA y por SEGMENTO social. Todo dato duro (cifra, fecha) debe venir de las fuentes crudas.`
    : skill === 'socioafectiva'
...
- semiotica.miedos, semiotica.deseos, semiotica.necesidades: EXACTAMENTE 10 elementos cada lista (rank 1 a 10), nunca menos.`,
  ...
  tensiones: `
REQUISITOS MÍNIMOS DE CANTIDAD (TENSIONES) — mínimo 6, ideal hasta 10, en TODAS estas listas: ranking, emociones, narrativas, territorios, riesgos, trayectoria, alertas. Nunca entregues menos de 6 en ninguna. Además: semaforo EXACTAMENTE 5, emocionesTendencia = mismo conteo que emociones, trayectoriaMeses EXACTAMENTE 5, aprobacionAlcalde es un único objeto (no lista).
...
- ranking: 6-10 tensiones sociales distintas, cada una completa (nunca "—" en ningún campo):
  ...
  · "evidencia" es OBLIGATORIO: un párrafo con 2-4 datos verificables (cifras, fechas, nombres de colonias) rematando cada dato con la fuente entre paréntesis, ej: "Corte de hasta 96h en mayo por rehabilitación planta Tezontle y acueducto Tizayuca (La Silla Rota, ICF 4.3) - Colonias Carboneras, El Saucillo... (ICF 4.0)". Encadena 2-3 datos con guiones " - " como en el ejemplo, no lo dejes en una sola oración vaga.
  · "lecturaEstrategica" es OBLIGATORIO y es EL CAMPO MÁS DETALLADO de cada tensión (80-140 palabras): interpreta el hallazgo, lo conecta con otros hechos/tensiones del mismo periodo, señala qué actor institucional queda expuesto y qué puede pasar si el patrón continúa. Es análisis, no un resumen de "evidencia".
  · "recomendacion" debe ser una acción específica y ejecutable en el corto plazo, no un consejo genérico.

- semaforo: EXACTAMENTE 5 indicadores, en este orden: "Tensión Social" (valor = score de la tensión #1 del ranking), "Reputacional" (valor coherente con el promedio de "srr" en riesgos), "Escalamiento" (probabilidad de que la tensión #1 se agrave), "Confianza Institucional" (etiqueta ALTA/MEDIA/MEDIA-BAJA/BAJA, con "detalle" citando una cifra de aprobación o encuesta si existe), "Movilización Social" (etiqueta ALTA/MEDIA/BAJA, con "detalle" indicando si hay o no protesta de calle documentada). Cada uno con "banda" (ej. "Roja / Crítica", "Naranja / Alto") y "detalle" concreto.
...
- emociones: 6-8 emociones (ej. Hartazgo, Indignación, Desconfianza, Miedo, Ansiedad, Resignación, Esperanza — usa las que apliquen al territorio), cada una con:
  ...
  · "porcentaje": participación relativa 0-100 de cada emoción dentro del total (para la gráfica de dona); la suma de todas debe aproximarse a 100.

- emocionesTendencia: UNA entrada por CADA emoción de "emociones" (mismo "nombre", mismo orden), con "tendencia" (símbolo de dirección + calificativo, ej. "↑ Fuerte ascenso", "— Estable alto", "↓ En descenso") y "detalle" (una línea del hecho concreto que explica esa tendencia respecto al periodo anterior).

- emocionesLectura: párrafo de 150-220 palabras que interprete el conjunto del mapa emocional (no una sola emoción aislada), explicando en qué se ancla cada emoción dominante y qué combinación resulta más peligrosa/movilizable estratégicamente.

- narrativas: 6-10, cada una con "politica" (impacto/potencial político específico, no genérico), "potencialScore" (0-100, coherente con "politica" y variado entre narrativas) y "fuente" (medio + rango de fechas, ej. "La Silla Rota, mayo-julio 2026").

- territorios: 6-10 zonas/colonias específicas (nunca el municipio genérico completo), cada una con:
  · "nivel": "Crítico", "Alto" o "Medio", coherente con "color": "Rojo" (crítico), "Naranja" (alto) o "Amarillo" (medio).
  · "observaciones": párrafo de 40-70 palabras con AL MENOS una cifra concreta (número de personas/colonias/pesos), fecha y fuente entre paréntesis — nunca solo descriptivo sin datos, ej: "Más de 200 comerciantes retirados de plaza Constitución en abril 2026 y reubicados en obra de 5.6 mdp junto al mercado Benito Juárez. Protesta en junio por baja afluencia (Criterio Hidalgo / La Silla Rota, ICF 4.0-4.3)".

- riesgos: 6-10, EN EL MISMO ORDEN Y CANTIDAD que las tensiones del ranking (cada tensión del ranking debe tener su fila de riesgo correspondiente), cada uno con "actorExpuesto", "tipoSenal", "probEscalar" y "consecuencia" llenos (nunca vacíos) y "accion" concreta. "consecuencia" siempre incluye un plazo aproximado (ej. "en 30-45 días").

- trayectoriaMeses: EXACTAMENTE 5 etiquetas de mes/periodo reales y consecutivos, terminando en el mes/corte actual del análisis.

- trayectoria: DEBE cubrir las MISMAS 6-10 tensiones que aparecen en "ranking" (mismos nombres, mismo orden) — no un subconjunto reducido. Cada tensión trae 5 valores mensuales "m1" a "m5" alineados con "trayectoriaMeses". "m5" (actual) debe coincidir con el "score" de esa tensión en "ranking". "delta" = m5 - m1, con signo (+/-). "velocidad" refleja qué tan abrupto fue el cambio en el último tramo (Acelerada/Moderada/Gradual/Estable/Bajando).

- aprobacionAlcalde: un solo objeto (no lista) con el ÚNICO corte de aprobación verificado en encuesta pública real de las fuentes; si no hay ninguno, "valor":0 y "nota" explicando la ausencia de dato — nunca inventar ni interpolar cifras de aprobación.

- alertas: 6-10 alertas (como las demás secciones), y cada alerta debe ser MUY explicativa: usa estas filas en "rows" como mínimo (adapta la etiqueta si hace falta, pero cubre el fondo): "Territorio", "Emoción" (con intensidad X/5), "Actor expuesto", "Qué ocurrió" (párrafo de 40-80 palabras con fecha y hechos concretos), "Narrativa activa", "Fuente verificadora" (medio + fecha + ICF), "Riesgo" (nivel + SRR + contexto), "Escalamiento" (probabilidad + qué lo activaría), "Acción inmediata" (específica, no genérica). Mínimo 8 rows por alerta. Además, "comentariosMedios": 2-4 comentarios de prensa/redes sociales sobre ese hecho específico, cada uno con "cita" y "fuente" (medio o red + fecha aproximada).
